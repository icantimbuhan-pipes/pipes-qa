...
# Registry — add entry here when a provider is ready, uncomment the module path
PROVIDERS = [
    {
        "key":    "heavy-khomp",
    ...
    },
    {
        ...
        "module": "providers.heavy_fs",
        "active": False,   # flip to True after running /qa:add-provider
    },
    {
        "key":    "lite-khomp",
...
def _select_providers() -> list[dict]:
    """Show an interactive numbered menu. Returns the list of provider dicts to run."""
    console.print(Panel(
    ...
    while True:
        ...
        if choice.isdigit():
            ...
            if 0 <= idx < len(PROVIDERS):
                p = PROVIDERS[idx]
                if not p["active"]:
                    console.print(f"  [yellow]{p['name']} is not configured yet.[/yellow] Run [bold]/qa:add-provider[/bold] to set it up.")
                    continue
                return [p]
...
def _trigger_and_wait(provider, instruction: str = "") -> bool:
    ...
    Returns True (answered/ready) or False (skipped).
...
def main():
    # --provider flag skips the menu (useful for scripts/CI)
    provider_key = None
